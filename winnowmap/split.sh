...
echo -e "\t<input.fofn>: fastq.gz files we want to split by every 100 million lines."
...
if [[ ${fq##*.} == "gz" ]]; then
	echo "\
	zcat $fq | split -a 4 -d -l 10000000 --additional-suffix=.fq   - split/$fq_prefix."
	zcat $fq | split -a 4 -d -l 10000000 --additional-suffix=".fq" - split/$fq_prefix.
else
	echo "\
	split -a 4 -d -l 10000000 --additional-suffix=.fq   $fq split/$fq_prefix."
	split -a 4 -d -l 10000000 --additional-suffix=".fq" $fq split/$fq_prefix.
fi
