...
function run_one_iteration {
    ...
    ${MERFIN} -sequence ${in_draft} -seqmers ${out_meryl} -readmers ${in_readmers} -peak 106 -vcf ${out_racon_fasta}.vcf -output ${out_merfin} -memory1 12 -memory2 150 -vmer -threads ${threads} -disable-kstar
    ...
    ${MINIMAP2} -cx asm5 --cs ${out_prefix}.iter_0.consensus.fasta ${out_consensus} > ${out_consensus}.minimap2.paf
    ...
    ${PAFTOOLS} call -f ${out_prefix}.iter_0.consensus.fasta ${out_consensus}.minimap2.sorted.paf > ${out_consensus}.paftools.vcf
}
