...
include { WINNOWMAP_MAP    as WINNOWMAP_MAP_R2         } from '../modules/winnowmap'
include { WINNOWMAP_MERGE  as WINNOWMAP_MERGE_HIFI_R2  } from '../modules/winnowmap'
include { WINNOWMAP_MERGE  as WINNOWMAP_MERGE_ONT_R2   } from '../modules/winnowmap'
include { WINNOWMAP_FILTER as WINNOWMAP_FILTER_R2      } from '../modules/winnowmap'
include { SAM2PAF          as SAM2PAF_R2               } from '../modules/winnowmap'
include { BWA_MAP          as BWA_MAP_R2               } from '../modules/bwa'
include { BWA_MERGE        as BWA_MERGE_R2             } from '../modules/bwa'
...
workflow MAPPING_R2 {
    ...
    def mapped   = WINNOWMAP_MAP_R2( hifiInput.mix(ontInput) )
    def mappedByPlat = mapped.branch { hap, ver, plat, bam, bai ->
        hifi: plat == 'hifi'
        ont:  true
    }
    def mergedHifi = WINNOWMAP_MERGE_HIFI_R2( mappedByPlat.hifi.groupTuple(by: [0, 1, 2], size: nHifi) )
    def mergedOnt  = WINNOWMAP_MERGE_ONT_R2(  mappedByPlat.ont .groupTuple(by: [0, 1, 2], size: nOnt ) )
    def filtered = WINNOWMAP_FILTER_R2( mergedHifi.mix(mergedOnt) )
    SAM2PAF_R2(filtered)
    ...
    def bwaMapped = BWA_MAP_R2( bwa_refs.combine(allBwaReads) )
    def bwaMerged = BWA_MERGE_R2( bwaMapped.groupTuple(by: [0, 1, 2]) )
