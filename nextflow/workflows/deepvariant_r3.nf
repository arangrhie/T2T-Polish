/* Round-3 — per-round process aliases for NF 25.x uniqueness constraint */
include { MERGE_HYBRID        as MERGE_HYBRID_R3          } from '../modules/deepvariant'
include { PEPPER_MARGIN_DV    as PEPPER_MARGIN_DV_R3      } from '../modules/deepvariant'
include { DV_MERGE_CHR_VCFS   as DV_MERGE_CHR_VCFS_R3    } from '../modules/deepvariant'
include { DV_MAKE_EXAMPLES    as DV_MAKE_EXAMPLES_HYB_R3  } from '../modules/deepvariant'
include { DV_CALL_VARIANTS    as DV_CALL_VARIANTS_HYB_R3  } from '../modules/deepvariant'
include { DV_POSTPROCESS      as DV_POSTPROCESS_HYB_R3    } from '../modules/deepvariant'
include { DV_MAKE_EXAMPLES    as DV_MAKE_EXAMPLES_ONT_R3  } from '../modules/deepvariant'
include { DV_CALL_VARIANTS    as DV_CALL_VARIANTS_ONT_R3  } from '../modules/deepvariant'
include { DV_POSTPROCESS      as DV_POSTPROCESS_ONT_R3    } from '../modules/deepvariant'

workflow DEEPVARIANT_R3 {
    take:
    refs
    wm_bams
    bwa_bams
    ...
    def hyb_merged       = MERGE_HYBRID_R3(hybrid_input)
    ...
    def hyb_input        = hyb_merged_ref
        .map { hap, ver, combo, bam, bai, ref, gzi, fai ->
               tuple(hap, ver, combo, 'HYBRID_PACBIO_ILLUMINA',
                     ...
                     ref, gzi, fai, bam, bai) }
    def hyb_ex    = DV_MAKE_EXAMPLES_HYB_R3(hyb_input)
    ...
    def hyb_calls = DV_CALL_VARIANTS_HYB_R3(hyb_ex)
    DV_POSTPROCESS_HYB_R3(
    ...
    def r9_input = ( params.ont_chemistry == 'r9' )
        ...
        : Channel.empty()
    PEPPER_MARGIN_DV_R3(r9_input)
    def r9_merged = DV_MERGE_CHR_VCFS_R3( PEPPER_MARGIN_DV_R3.out.vcfs.groupTuple(by: [0, 1, 2]) )
    ...
    def ont_input = ( params.ont_chemistry != 'r9' )
        ...
        : Channel.empty()
    def ont_ex    = DV_MAKE_EXAMPLES_ONT_R3(ont_input)
    def ont_exref = ont_input.join(ont_ex, by: [0,1,2,3,4])
        .map { hap,ver,combo,mode,mq,ref,gzi,fai,bam,bai,examples_dir ->
               tuple(hap,ver,combo,mode,mq,ref,gzi,fai,examples_dir) }
    def ont_calls = DV_CALL_VARIANTS_ONT_R3(ont_ex)
    DV_POSTPROCESS_ONT_R3(
        ont_calls.join(ont_exref, by:[0,1,2,3,4])
            .map { hap,ver,combo,mode,mq,cvo,ref,gzi,fai,examples_dir ->
                   tuple(hap,ver,combo,mode,mq,ref,gzi,fai,cvo,examples_dir) }
    )

    def dv_vcfs_ch = DV_POSTPROCESS_HYB_R3.out[0]
        .mix( DV_POSTPROCESS_ONT_R3.out[0] )
        .mix( r9_merged )
