...
include { WINNOWMAP_MAP    as WINNOWMAP_MAP_R5         } from '../modules/winnowmap'
include { WINNOWMAP_MERGE  as WINNOWMAP_MERGE_HIFI_R5  } from '../modules/winnowmap'
include { WINNOWMAP_MERGE  as WINNOWMAP_MERGE_ONT_R5   } from '../modules/winnowmap'
include { WINNOWMAP_FILTER as WINNOWMAP_FILTER_R5      } from '../modules/winnowmap'
include { SAM2PAF          as SAM2PAF_R5               } from '../modules/winnowmap'
include { BWA_MAP          as BWA_MAP_R5               } from '../modules/bwa'
include { BWA_MERGE        as BWA_MERGE_R5             } from '../modules/bwa'
...
workflow MAPPING_R5 {
    ...
    def mapped   = WINNOWMAP_MAP_R5( hifiInput.mix(ontInput) )
    def mappedByPlat = mapped.branch { hap, ver, plat, bam, bai ->
        hifi: plat == 'hifi'
        ont:  true
    }
    def mergedHifi = WINNOWMAP_MERGE_HIFI_R5( mappedByPlat.hifi.groupTuple(by: [0, 1, 2], size: nHifi) )
    def mergedOnt  = WINNOWMAP_MERGE_ONT_R5(  mappedByPlat.ont .groupTuple(by: [0, 1, 2], size: nOnt ) )
    def filtered = WINNOWMAP_FILTER_R5( mergedHifi.mix(mergedOnt) )
    SAM2PAF_R5(filtered)
    ...
    def bwaMapped = BWA_MAP_R5( bwa_refs.combine(allBwaReads) )
    def bwaMerged = BWA_MERGE_R5( bwaMapped.groupTuple(by: [0, 1, 2]) )
