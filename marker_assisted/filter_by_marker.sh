...
if [[ "$#" -lt 5 ]]; then
  echo "Usage: filter_by_marker.sh alignment target asm marker.meryl len_filt [short-reads]"
  echo
  ...
  echo -e "\tshort-reads: T if provided alignment is from short-reads. F if from long-reads. DEFAULT=F. OPTIONAL"
  exit 0
fi
...
echo "
Extract $target aslignments from $alignment, split by $NUM_READS_PER_FILE unique read ids"
echo
...
if [ -e $target.markersandlength.cram ]; then
  ...
  do
    echo "
    $SCRIPT/convert.sh $target $asm $i"
    $SCRIPT/convert.sh $target $asm $i
    echo
  done
  ...
  echo "Merge"
  echo "
  $SCRIPT/merge.sh $target.bam $target $asm $meryldb $len_filt"
  $SCRIPT/merge.sh $target.bam $target $asm $meryldb $len_filt
  echo
fi
